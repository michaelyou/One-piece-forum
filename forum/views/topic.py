...
def get_index(request):
    ...
    if user.is_authenticated():
        counter = {
            'topics': user.topic_author.all().count(),   #都是外键，到topic类下去找这个属性，注意别名
            'replies': user.reply_author.all().count(),
            'favorites': user.fav_user.all().count()
        }
        notifications_count = user.notify_user.filter(status=0).count()   #notify_user也是一个外键
    ...
    active_page = 'topic'
    return render_to_response('topic/topics.html',
        context=RequestContext(request, locals()))


def get_topic_view(request, topic_id, errors=None):
    try:
...
@login_required
def post_topic_view(request, topic_id):
    try:
    ...
    if not form.is_valid():
        return get_topic_view(request, topic_id, errors=form.errors)
    ...
    if last_reply:
        ...
        if last_replied_fingerprint == new_replied_fingerprint:
            ...
            return get_topic_view(request, topic.id, errors=errors)
...
@login_required
def get_topic_create(request, slug=None, errors=None):
...
def post_topic_create(request, slug=None):
    ...
    if not form.is_valid():
        return get_topic_create(request, slug=slug, errors=form.errors)
    ...
    if last_created: # 如果用户最后一篇的标题内容与提交的相同
        ...
        if last_created_fingerprint == new_created_fingerprint:
            ...
            return get_topic_create(request, slug=slug, errors=errors)
...
@login_required
def get_topic_delete(request, topic_id):
...
@login_required
def get_topic_edit(request, topic_id, errors=None):
    topic = get_object_or_404(Topic, pk=topic_id)
...
@login_required
def post_topic_edit(request, topic_id):
    topic = get_object_or_404(Topic, pk=topic_id)

    form = CreateForm(request.POST)
    if not form.is_valid():
        return get_topic_edit(request, topic_id, errors=form.errors)

    user = request.user
    if topic.author_id != user.id:
        errors = {'invalid_permission': [u'没有权限修改该主题']}
        return get_topic_edit(request, topic_id, errors=errors)
...
@login_required
def get_reply_delete(request, reply_id):

    try:
        topic_id = int(request.GET.get('topic_id'))  #注意这里的topic_id参数是在html中指定的
    ...
    return redirect('/t/%s/' % topic_id)  #重定向到删除的topic页面
